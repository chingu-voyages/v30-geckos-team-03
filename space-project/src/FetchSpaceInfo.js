  ...
  async componentDidMount() {
    ...
    let astros = values.map(function (d) {
      ...
      for (let myitem of myiterator) {
        return (
          "****ASTEROID INFORMATION Name: " +
          myitem.name +
          " Orbiting: " +
          myitem.close_approach_data[0].orbiting_body +
          " JPL ID: " +
          myitem.id +
          " Close Approach Date: " +
          myitem.close_approach_data[0].close_approach_date_full +
          " Dangerous?: " +
          myitem.close_approach_data.is_potentially_hazardous_asteroid +
          " Distance in Miles: " +
          ...
          myitem.close_approach_data[0].relative_velocity.kilometers_per_hour +
    ...
    let astro_object_list = values.map(function (g) {
    ...
    this.setState({
      information: data.near_earth_objects,
      astros: astros,
      astro_list_compiled: astro_list_compiled,
      hazard: hazard,
      loading: false,
    });
  }
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <div>
          Potentially Hazardous? Boolean:{JSON.stringify(this.state.hazard)}
        </div>
